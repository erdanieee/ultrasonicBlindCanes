...
SR04::SR04(uint8_t trigger_pin, uint8_t echo_pin, int max_cm_distance) {
...
unsigned int SR04::ping_in() {
	unsigned int echoTime = SR04::ping();          // Calls the ping method and returns with the ping echo distance in uS.
	return SR04Convert(echoTime, US_ROUNDTRIP_IN); // Convert uS to inches.
}


unsigned int SR04::ping_cm() {
	unsigned int echoTime = SR04::ping();          // Calls the ping method and returns with the ping echo distance in uS.
	return SR04Convert(echoTime, US_ROUNDTRIP_CM); // Convert uS to centimeters.
}


unsigned int SR04::ping_median(uint8_t it) {
...
void SR04::ping_timer(void (*userFunc)(void)) {
	if (!ping_trigger()) return;         // Trigger a ping, if it returns false, return without starting the echo timer.
	timer_us(ECHO_TIMER_FREQ, userFunc); // Set ping echo timer check every ECHO_TIMER_FREQ uS.
}

 
boolean SR04::check_timer() {
...
void SR04::timer_us(unsigned int frequency, void (*userFunc)(void)) {
...
void SR04::timer_ms(unsigned long frequency, void (*userFunc)(void)) {
	timer_setup();                       // Configure the timer interrupt.
	intFunc = SR04::timer_ms_cntdwn;  // Timer events are sent here once every ms till user's frequency is reached.
...
void SR04::timer_stop() { // Disable timer interrupt.
#if defined (__AVR_ATmega32U4__) // Use Timer4 for ATmega32U4 (Teensy/Leonardo).
	TIMSK4 = 0;
#else
	TIMSK2 &= ~(1<<OCIE2A);
...
unsigned int SR04::convert_in(unsigned int echoTime) {
	return SR04Convert(echoTime, US_ROUNDTRIP_IN); // Convert uS to inches.
}


unsigned int SR04::convert_cm(unsigned int echoTime) {
	return SR04Convert(echoTime, US_ROUNDTRIP_CM); // Convert uS to centimeters.
}
